...
trait EmptyResponseGenerator[F[_], G[_]] extends Any with ResponseGenerator {
  def apply(headers: Header*)(implicit F: Applicative[F]): F[Response] =
...
trait EntityResponseGenerator[F[_], G[_]] extends Any with ResponseGenerator {
  ...
  def apply(headers: Header*)(implicit F: Applicative[F]): F[Response] =
    F.pure(Response(status, headers = Headers(`Content-Length`.zero :: headers.toList)))

  def apply[A](body: G[A])(implicit F: Monad[F], w: EntityEncoder[G, A]): F[Response] =
    F.flatMap(liftG(body))(apply[A](_))

  def apply[A](body: A, headers: Header*)(
      implicit F: Applicative[F],
      w: EntityEncoder[G, A]): F[Response] = {
...
trait LocationResponseGenerator[F[_], G[_]] extends Any with EntityResponseGenerator[F, G] {
  ...
  def apply(location: Uri)(implicit F: Applicative[F]): F[Response] =
    F.pure(
      Response(
        status = status,
        headers = Headers(`Content-Length`.zero :: Location(location) :: Nil)))

  def apply(location: Location, headers: Header*)(implicit F: Applicative[F]): F[Response] =
    F.pure(
      Response(status, headers = Headers(`Content-Length`.zero :: location :: headers.toList)))
...
trait WwwAuthenticateResponseGenerator[F[_], G[_]] extends Any with ResponseGenerator {
  ...
  def apply(challenge: Challenge, challenges: Challenge*)(
      implicit F: Applicative[F]): F[Response] =
    F.pure(
      Response(
        status = status,
        headers =
...
trait AllowResponseGenerator[F[_], G[_]] extends Any with ResponseGenerator {
  def apply(allow: Allow, headers: Header*)(implicit F: Applicative[F]): F[Response] =
