...
private final class PoolManager[F[_], A <: Connection[F]](
  ...
  def borrow(key: RequestKey): F[NextConnection] =
    F.asyncF { callback =>
      SemaphoreUtil.withPermit(semaphore) {
        if (!isClosed) {
          def go(): F[Unit] =
  ...
  /**
    ...
    */
  def release(connection: A): F[Unit] = SemaphoreUtil.withPermit(semaphore) {
  ...
  /**
    ...
    * @return An effect of Unit
    */
  override def invalidate(connection: A): F[Unit] = SemaphoreUtil.withPermit(semaphore) {
  ...
  /**
    ...
    * @return An effect Of Unit
    */
  def shutdown(): F[Unit] = SemaphoreUtil.withPermit(semaphore) {
    F.delay {
